...
import BatchEntryModal from '@/components/modals/BatchEntryModal'
import ToolsModal from '@/components/modals/ToolsModal'
import PrintPreviewModal from '@/components/modals/PrintPreviewModal/index'
...
export default function DashboardPage() {
    ...
    // ✅ Fetch available years and validate current year
    useEffect(() => {
        const fetchAvailableYears = async () => {
            try {
                ...
                if (data.length === 0) {
                    ...
                    await fetch(`/api/init-year/${currentYear}`, { method: 'POST' })
                    setAvailableYears([currentYear])
                    setSelectedYear(currentYear)
                    localStorage.setItem('mainForm2_selectedYear', currentYear.toString())
                    setIsLoadingYear(false)
                    return
                }

                // ✅ Check if selected year exists
                if (!data.includes(selectedYear)) {
                    // Find closest year (largest available)
                    const sortedYears = data.sort((a: number, b: number) => a - b)
                    const closestYear = sortedYears[sortedYears.length - 1]
                    console.log(`⚠️ Year ${selectedYear} not found, using ${closestYear}`)
                    setSelectedYear(closestYear)
                    localStorage.setItem('mainForm2_selectedYear', closestYear.toString())
                }
    ...
    // ===== REORDER AA =====
    const handleReorderAA = async () => {
        if (!selectedChapterId) {
            alert('Παρακαλώ επιλέξτε ένα κεφάλαιο')
            return
        }
        ...
        if (!confirm(`Θέλετε να αναδιατάξετε τους ΑΑ για το κεφάλαιο "${chapterName}";`)) {
            return
        }

        setSaving(true)
        try {
            const response = await fetch(`/api/chapters/${selectedChapterId}/reorder-aa?year=${selectedYear}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            })
    ...
    // Effects
    useEffect(() => {
        if (!isLoadingYear && selectedYear) {
            fetchChapters()
        }
    }, [selectedYear, isLoadingYear])
    ...
    // ✅ Show loading while validating year
    if (isLoadingYear) {
        return (
    ...
    // ✅ If no years available
    if (availableYears.length === 0) {
        return (
            <div className="h-full flex flex-col items-center justify-center gap-4">
                <div className="text-4xl">📅</div>
                <h1 className="text-2xl font-bold">Δεν υπάρχουν διαθέσιμα έτη</h1>
                <p className="text-gray-500">Παρακαλώ δημιουργήστε ένα νέο έτος</p>
                <button
                    onClick={() => window.location.href = '/year-opening-wizard'}
        ...
        <div style={{ display: 'flex', height: '100vh', width: '100vw', overflow: 'hidden' }}>
                        ...
                        <tfoot style={{
                            ...
                            <tr className={`${darkMode ? 'bg-blue-900/60' : 'bg-blue-200'} font-bold border-t-2 border-blue-500`}>
                                ...
                                {displayOnomastika.map((o, idx) => {
                                    ...
                                    const balance = (Number(prevBalance.debit) + Number(currentDebit)) - (Number(prevBalance.credit) + Number(currentCredit))


                                    console.log(`🔍 ${o.name || 'unknown'}: prevDebit=${prevBalance.debit}, prevCredit=${prevBalance.credit}, currentDebit=${currentDebit}, currentCredit=${currentCredit}, balance=${balance}`)
            ...
            <ToolsModal
                ...
                onMessage={(type, text) => {
                    ...
                    else if (type === 'warning') alert('⚠️ ' + text)
                }}
            />
