...
export default function DashboardPage() {
    ...
    const [frozenColumns, setFrozenColumns] = useState(true)  // ✅ ΠΡΟΣΘΗΚΗ
    const [backupPath, setBackupPath] = useState('C:\\Backup_Biblio')



    // Στο page.tsx, βρείτε το useEffect που φορτώνει τα settings και προσθέστε:
    ...
    const displayOnomastika = useMemo(() => {
        ...
        // Αν δεν υπάρχουν ονομαστικά, δημιούργησε placeholders
        ...
        // Φιλτράρισμα πραγματικών ονομαστικών (όχι placeholders)
        let realOnomastika: any[] = []

        if (!selectedPage) {
            // Χωρίς επιλογή σελίδας, πάρτε όλα τα πραγματικά ονομαστικά
        ...
        // Συμπλήρωσε με placeholders μέχρι 16
    ...
    const fetchChapterPages = async () => {
        ...
        try {
            const res = await fetch(`/api/chapters/${selectedChapterId}?year=${selectedYear}`)  // ✅ ΠΡΟΣΘΗΚΗ YEAR
            const data = await res.json()
            if (data.page && data.page !== '') {
                ...
                if (savedPage && pages.includes(parseInt(savedPage))) {
                    ...
                    // ✅ Αν δεν υπάρχει αποθηκευμένη σελίδα, πάρε την πρώτη
    ...
    // In dashboard/page.tsx - update fetchChapterData
    ...
    const fetchChapterData = async () => {
        ...
        try {
            ...
            // ✅ FIX: Load records and their entries
            ...
            // ✅ FIX: Load number values from entries
            ...
            // ✅ Load attachments
            // ✅ Load attachments with year parameter
    ...
    // ✅ saveOnomastikoEdit - ΠΡΟΣΘΗΚΗ YEAR
    const saveOnomastikoEdit = async () => {
        ...
        try {
            ...
            // ✅ ΠΡΟΣΘΗΚΗ year στο query parameter
    ...
    // ✅ deleteOnomastiko - ΠΡΟΣΘΗΚΗ YEAR
    ...
    const saveEditedRecord = async () => {
        ...
        try {
            ...
            updatedValues[`${recordId}_aa`] = editAa
            updatedValues[`${recordId}_day`] = editDay
            updatedValues[`${recordId}_month`] = editMonth
            updatedValues[`${recordId}_description`] = editDescription
            ...
            editEntries.forEach(entry => {
                ...
                const existingDebit = numberValues[debitKey] || ''
                const existingCredit = numberValues[creditKey] || ''

                updatedValues[debitKey] = (entry.debit !== undefined && entry.debit !== '')
                    ? entry.debit
                    : existingDebit
                updatedValues[creditKey] = (entry.credit !== undefined && entry.credit !== '')
                    ? entry.credit
                    : existingCredit
            })
    ...
    const loadAttachments = async (recordId: number) => {
        try {
            // ✅ Προσθήκη year στο query parameter
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', width: '100vw', overflow: 'hidden' }}>
                        ...
                        <div className="flex items-center justify-between w-full">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    onClick={handleSave}
                                    disabled={saving}
                                    className={`px-3.5 py-1 rounded-md text-sm font-medium text-white transition-colors ${saving ? 'opacity-50' : ''} ${darkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-600 hover:bg-blue-700'}`}
                                >
            ...
            <SettingsModal
                ...
                frozenColumns={frozenColumns}          // ✅ ΠΡΟΣΘΗΚΗ
                setFrozenColumns={setFrozenColumns}    // ✅ ΠΡΟΣΘΗΚΗ
                sidebarOpen={sidebarOpen}
                setSidebarOpen={setSidebarOpen}
